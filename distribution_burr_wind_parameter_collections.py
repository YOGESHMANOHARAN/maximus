...
distr_output = pd.DataFrame(columns=['date','burr_c','burr_d','burr_loc','burr_scale'])
date_list=[]
dist_list_c=[]
dist_list_d=[]
dist_list_scale=[]
        ...
        for k in range(0, 24):
            for l in range(0,4):
                try:
                    ...
                    solar_dist = Fitter(solar_irrad,distributions=['burr'])
                    solar_dist.fit()
                    solar_dist.summary()
                    result = solar_dist.get_best(method='sumsquare_error')
                    date_list.append(str(month[i]) + '/' + str(day[j]) + ' ' + time)
                    dist_list_c.append(result['burr']['c'])
                    dist_list_d.append(result['burr']['d'])
                    dist_list_loc.append(result['burr']['loc'])
                    dist_list_scale.append(result['burr']['scale'])
                except:
                    if 2>>0:
                        print(day[j],month[i])
                        continue